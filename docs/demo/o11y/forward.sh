...
# in the o11y demo, mapped to the same local port. Useful when the
...
cleanup() {
  ...
  # Kill any backgrounded children. `xargs -r` is GNU-only; use a bash
  # array test instead so this is portable to BSD/macOS xargs.
  local pids=()
...
info "Forwarding o11y NodePorts (address=${ADDRESS}; Ctrl-C to stop)"

# svc/grafana — NodePort 31800, port 80, targetPort 3000.

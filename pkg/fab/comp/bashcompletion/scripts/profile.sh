...
# Show Hedgehog resources
hres() {
    echo -e "${HEDGEHOG_COLOR}🦔 Hedgehog Resources:${RESET_COLOR}"
    kubectl api-resources | egrep "SHORTNAMES|githedgehog.com" | sort | while read -r line; do
        if [[ "$line" =~ ^NAME ]]; then
...
echo -e "  ${HEDGEHOG_COLOR}hres${RESET_COLOR}       - show Hedgehog Resources"

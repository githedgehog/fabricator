...
func discoverEndpoints(
	...
	seen := map[connmatrix.EndpointKey]bool{}
	for _, name := range serverNames {
...
func matchEndpointIPs(server string, addrs []ifaceAddr, attachments []subnetAttachment) []connmatrix.Endpoint {
	endpoints := []connmatrix.Endpoint{}
	for _, att := range attachments {
		ep := connmatrix.Endpoint{
			Server:  server,
			Subnet:  att.FullName,
			HostBGP: att.HostBGP,
		}

...
// ReachabilityReason is a short tag identifying why a given reachability
// decision holds (peering type, isolation, default-deny, etc.).
...
// PickExpectation selects the first proto-agnostic ALLOW entry from a list,
// or an implicit-DENY if none exists. It captures the "default reachability"
// pattern used by the executor and legacy callers that do not care about
// protocol/port granularity: a proto-agnostic ALLOW overrides any implicit
// DENY, while DENY entries yield to ALLOW ones in the same bucket.

...
func createLinkSpeedLayer(model *MxGraphModel) {
	...
	for _, edgeData := range edgePositions {
		...
		if srcNOS != "" {
			// Shorten "Ethernet" to "Eth" for display
			displaySrcNOS := strings.Replace(srcNOS, "Ethernet", "Eth", 1)
			// Add padding spaces for short labels (5 chars or less) to cover base layer labels
			if len(displaySrcNOS) <= 5 {
				displaySrcNOS = " " + displaySrcNOS + " "
			}
			srcNOSText := fmt.Sprintf("<span style=\"font-size:10px;\">%s</span>", displaySrcNOS)
			srcNOSWidth := len(displaySrcNOS)*4 + 8
		...
		if tgtNOS != "" {
			// Shorten "Ethernet" to "Eth" for display
			displayTgtNOS := strings.Replace(tgtNOS, "Ethernet", "Eth", 1)
			// Add padding spaces for short labels (5 chars or less) to cover base layer labels
			if len(displayTgtNOS) <= 5 {
				displayTgtNOS = " " + displayTgtNOS + " "
			}
			tgtNOSText := fmt.Sprintf("<span style=\"font-size:10px;\">%s</span>", displayTgtNOS)
			tgtNOSWidth := len(displayTgtNOS)*4 + 8

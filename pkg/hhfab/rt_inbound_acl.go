...
func externalInboundACLTest(ctx context.Context, testCtx *VPCPeeringTestCtx, _ *ConnectivityMatrix) (bool, []RevertFunc, error) {
...
// failure.
func externalInboundUserACLTest(ctx context.Context, testCtx *VPCPeeringTestCtx, _ *ConnectivityMatrix) (bool, []RevertFunc, error) {
	target, err := pickInboundACLTarget(ctx, testCtx.kube, testCtx.vlab)
	if err != nil {
		return errors.Is(err, errNoInboundACLProbeSource), nil, err
	}

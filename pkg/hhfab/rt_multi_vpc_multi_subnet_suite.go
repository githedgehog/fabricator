...
type staticExternalSelection struct {
	...
	// attach is conn's only VPCAttachment, the one the test deletes and later restores
...
func selectStaticExternalTarget(ctx context.Context, kube kclient.Client, candidates []wiringapi.Connection, vpcList *vpcapi.VPCList) (*staticExternalSelection, error) {
	...
	if err := kube.List(ctx, attachList, kclient.InNamespace(kmetav1.NamespaceDefault)); err != nil {
		return nil, fmt.Errorf("listing VPCAttachments: %w", err)
	}
	connList := &wiringapi.ConnectionList{}
	if err := kube.List(ctx, connList, kclient.InNamespace(kmetav1.NamespaceDefault)); err != nil {
	...
	attachOfConn := make(map[string]*vpcapi.VPCAttachment, len(attachList.Items))
	...
	for i := range candidates {
		...
		attach := attachOfConn[c.Name]
		sel := &staticExternalSelection{conn: c, attach: attach, targetVPC: attach.Spec.VPCName(), routeCheckSw: map[string]bool{}}
...
func staticExternalTest(ctx context.Context, testCtx *VPCPeeringTestCtx, _ *ConnectivityMatrix) (bool, []RevertFunc, error) {
	...
	// the selection already resolved conn's only VPCAttachment, reuse it so the attachment the test
	// deletes is exactly the one the candidate was accepted on
	vpcAtt := *sel.attach

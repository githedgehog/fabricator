...
func hostBGPTest(ctx context.Context, testCtx *VPCPeeringTestCtx, _ *ConnectivityMatrix) (bool, []RevertFunc, error) {
	reverts := []RevertFunc{}
	...
	for _, vpcName := range []string{hbgpVPCAName, hbgpVPCBName} {
		...
		hbgp.Endpoint = &Endpoint{Server: &ServerEndpoint{
			Name:    mhServer.Server.Name,
			VPC:     vpcName,
			Subnet:  "subnet-01",
			HostBGP: true,
		}}
	...
	for _, regularVPC := range []*RegularVPC{&regularVPCA, &regularVPCB} {
		regularVPC.Endpoint = &Endpoint{Server: &ServerEndpoint{
			Name:   regularVPC.Server.Name,
			VPC:    regularVPC.VPC.Name,
			Subnet: regularSubnetID,
		}}
	}
	...
	g.Go(func() error {
		...
		for line := range strings.Lines(stdout) {
			...
			case hostBGP.A.Subnet.Contains(prefix.Addr()):
				hostBGP.A.Endpoint.Server.IP = prefix.Addr()
			case hostBGP.B.Subnet.Contains(prefix.Addr()):
				hostBGP.B.Endpoint.Server.IP = prefix.Addr()
			default:
				slog.Warn("hostBGP VIP does not match any of the VPCs subnets, ignoring it", "VIP", prefix.String())
			}
		}
		for _, hbgp := range []HostBGPVPC{hostBGP.A, hostBGP.B} {
			if !hbgp.Endpoint.Server.IP.IsValid() {
				return fmt.Errorf("no hostBGP VIP from %s for %s", hbgp.Subnet, hbgp.VPCName) //nolint:err113
			}
		}
	...
	for _, regularVPC := range []*RegularVPC{&regularVPCA, &regularVPCB} {
		g.Go(func() error {
			...
			regularVPC.Endpoint.Server.IP = prefix.Addr()
	...
	for _, exp := range []struct {
		...
		{hostBGP.A.Endpoint, regularVPCA.Endpoint, peeringA.Name},
		{hostBGP.B.Endpoint, regularVPCB.Endpoint, peeringB.Name},
		{hostBGP.A.Endpoint, regularVPCB.Endpoint, ""},
		{hostBGP.B.Endpoint, regularVPCA.Endpoint, ""},
	} {
		verdict, reason := VerdictDeny, ReachabilityReason("")
		if exp.peering != "" {
			verdict, reason = VerdictAllow, ReachabilityReasonSwitchPeering
		}
		for _, pair := range []EndpointPair{
			{Source: exp.src, Destination: exp.dst},
			{Source: exp.dst, Destination: exp.src},
		} {

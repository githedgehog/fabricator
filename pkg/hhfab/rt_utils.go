...
const (
	// defaultDatapathConvergeTimeout bounds waitForDatapathConverged. The convergence tail it has
	// to absorb is much shorter: 12s for the gateway BGP/EVPN resync after an frr pod restart in
	// githedgehog/internal#494.
	//
...
// convergeStableRounds consecutive rounds, or timeout elapses. A pod or agent reporting Ready does
// not mean the dataplane is forwarding: a restarted gateway's BGP session still has to
// re-establish and its EVPN routes to reinstall. A strict probe run immediately afterward can
// catch that tail and lose its first packets.
//
// Use it for a restart or reload the test itself performs, where the reconvergence is expected.
// Do not use it to absorb loss the fabric is not supposed to produce: it would hide the defect
// rather than wait for a known settling period.
//
...
// NATMode specifies the type of NAT to use
type NATMode int

const (
	// NATModeStatic uses static (1:1) NAT mapping
...
type GwPeeringOptions struct {
	...
	VPC2NATCIDR []string
	// VPC1NATMode specifies the NAT mode for VPC1 (default: static)
	VPC1NATMode NATMode

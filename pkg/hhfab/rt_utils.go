...
// mode until it passes for convergeStable consecutive rounds or the timeout
// elapses. Route presence in leaf RIBs does not mean the dataplane (ASIC,
// neighbor resolution, gateway VTEP) is forwarding yet, so the first packets of
// the strict probe can hit the reconvergence tail and drop. Requiring several
// consecutive clean rounds confirms the path is stably forwarding, not just
// momentarily clean during a flap, before the strict probe runs. The strict
// probe that follows still runs once and tolerates no loss, so steady or
// intermittent loss is still caught. A path that never converges fails here.

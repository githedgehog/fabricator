...
`HHFAB_VLAB_COLLECT_FORCE` env var) forces show-tech collection on all
exits including success.

## Output layout

```
show-tech-output/
  runner-show-tech.log         # CI runner host diagnostics
...
`run-vlab.yaml` sets `HHFAB_VLAB_COLLECT=true` so failures always collect
show-tech. It exposes a `collect_show_tech` input (default `false`) that
maps to `HHFAB_VLAB_COLLECT_FORCE`, enabling collection on success too.

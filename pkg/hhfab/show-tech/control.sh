...
# ---------------------------
# No `kubectl logs --all-containers` here: it blocks on PodInitializing pods.
{
...
# ---------------------------
{
    ...
    # awk-filter: kubectl JSONPath can't filter on array elements.
    ...
    if [ ! -s /tmp/not-ready-pods.txt ]; then
        ...
        while IFS=$'\t' read -r ns pod; do
            ...
            # --request-timeout bounds each log call against PodInitializing stalls.
...
# ---------------------------
# show-tech is uploaded as a CI artifact; strip tokens/passwords from configs.
redact_secrets() {
    sed -E \
...
# ---------------------------
{
    ...
    # Fabricator CR carries inline Users[].Password and registry creds.
    for resource in $resources_githedgehog; do

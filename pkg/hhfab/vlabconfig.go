	...
	for _, conn := range conns.Items {
		if conn.Spec.Fabric != nil { //nolint:gocritic
			...
			for _, extAttach := range externalAttachs.Items {
				...
				if !ok {
					return nil, fmt.Errorf("external attachment %q has no associated VRF for the external %q", extAttach.Name, extName) //nolint:goerr113
				}
				if extVrf.ASN == 0 {
				...
				if extAttach.Spec.Switch.VLAN == 0 {
					if extNicCfg.Untagged {
						return nil, fmt.Errorf("multiple untagged attachments for the same external %s", extName) //nolint:goerr113
					}
				...
				if tagged && untagged {
					return nil, fmt.Errorf("both tagged and untagged attachments for the same external connection %s, this is not supported", conn.Name) //nolint:goerr113
				}

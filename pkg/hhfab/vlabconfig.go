...
func createVLABConfig(ctx context.Context, controls []fabapi.ControlNode, nodes []fabapi.Node, wiring client.Reader) (*VLABConfig, error) {
	...
	for _, conn := range conns.Items {
		if conn.Spec.Fabric != nil {
			...
			if len(connSplits) != 3 {
				slog.Warn("unexpected external connection name format, cannot derive external/switch", "connection", conn.Name)
				cfg.ExtInvalid = true
			...
			if !exists {
				slog.Warn("external connection has no matching external VRF", "connection", conn.Name)
				cfg.ExtInvalid = true
			...
			if err := wiring.Get(ctx, client.ObjectKey{Name: fmt.Sprintf("%s--%s", switchName, extName), Namespace: conn.Namespace}, extAttach); err != nil {
				slog.Warn("failed to get external attachment", "external", extName, "switch", switchName, "error", err)
				cfg.ExtInvalid = true
